...
detector_type = st.sidebar.selectbox(
    "Detector Backend",
    options=["onnx", "openvino", "yolov5", "rtdetr", "ultralytics"],
    index=0
)
...
enable_lpr = st.sidebar.checkbox("License Plate Recognition (OCR)", value=False)
...
if start_button and video_path:
    ...
    plate_detector = LicensePlateDetector() if enable_lpr else None
    plate_recognizer = LicensePlateRecognizer(cfg.ocr_engine, cfg.allow_synth_fallback) if enable_lpr else None
    db = VehicleDatabase()
