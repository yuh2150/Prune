...
class VehicleDetector:
    ...
    def _init_detector(self):
        ...
        if self.detector_type == "openvino":
            try:
                import openvino as ov
                print("[Detector] Loading model with OpenVINO...")
                core = ov.Core()
                model = core.read_model(self.weights)
                self.compiled_model = core.compile_model(model, "CPU")
                self.input_layer = self.compiled_model.input(0)
                self.output_layer = self.compiled_model.output(0)
                print("[Detector] Loaded OpenVINO model successfully.")
            except ImportError:
                print("[Detector] OpenVINO not installed! Falling back to ONNX Runtime...")
