...
def test(data,
    ...
    if training:  # called by train.py
        ...
        # Load modified and original model
        if opt.modification == "prune-unstructured":
        ...
        # SA for flops and params
        stride = max(int(model.stride.max()), 32) if hasattr(model, 'stride') else 32
        img = torch.zeros((1, model.yaml.get('ch', 3), stride, stride), device=next(model.parameters()).device)  # input
        flops = profile(deepcopy(model), inputs=(img,), verbose=False)[0] / 1E9 * 2  # stride GFLOPS
        img_size = imgsz if isinstance(imgsz, list) else [imgsz, imgsz]  # expand if int/float
        fs = round(flops * img_size[0] / stride * img_size[1] / stride, 3)  # 640x640 GFLOPS
    ...
    if training:
        ...
        return (mp, mr, map50, map, *(loss.cpu() / len(dataloader)).tolist()), maps, t, params, fs
...
if __name__ == '__main__':
    ...
    # python test.py --data data/coco.yaml --img 640 --batch 32 --conf 0.001 --iou 0.65 --task pruning_sensitivity_analysis --device 0 --weights yolov7.pt --name yolov7_640_sensitivity --modification prune-structured --prune-output output.txt
