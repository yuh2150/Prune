...
try:
    ...
    class DummyWandbLogger:
        def __init__(self, *args, **kwargs):
            self.wandb = None
            self.data_dict = args[3] if len(args) > 3 else (kwargs.get('data_dict', {}) if 'data_dict' in kwargs else {})
...
def train(hyp, opt, device, tb_writer=None):
    ...
    if rank in [-1, 0]:
        ...
        if weights.endswith('.pt') and os.path.isfile(weights):
            try:
                ckpt_temp = torch.load(weights, map_location=device, weights_only=False)
            except TypeError:
                ckpt_temp = torch.load(weights, map_location=device)
            run_id = ckpt_temp.get('wandb_id')
        else:
            run_id = None
    ...
    if pretrained:
        ...
        # Preventing the use of config file for iterative pruning
        model = Model(opt.cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device) if opt.cfg \
            else ckpt['model']  # create
        exclude = ['anchor'] if (opt.cfg or hyp.get('anchors')) and not opt.resume else []  # exclude keys
        state_dict = ckpt['model'].float().state_dict()  # to FP32
        state_dict = intersect_dicts(state_dict, model.state_dict(), exclude=exclude)  # intersect
        model.load_state_dict(state_dict, strict=False)  # load
        # Assigning pruning params
        if len(opt.pruning_params) > 0:
            pruning_params_parsed = ast.literal_eval(opt.pruning_params)
            prune_structured(model, pruning_params_parsed, opt.criterion, tiny=True) if 'tiny' in weights else \
                prune_structured(model, pruning_params_parsed, opt.criterion)
